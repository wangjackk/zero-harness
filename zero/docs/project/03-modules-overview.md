...
builtin skills:`routine-sdk`(框架 API)/ `zero-dev`(项目约定)/ `agent-messaging` / `skill-creator`。prime 专属另有 `hub_routine` / `routine_bridge`(python 包 skill,见 `agents/prime/skills/`)。
