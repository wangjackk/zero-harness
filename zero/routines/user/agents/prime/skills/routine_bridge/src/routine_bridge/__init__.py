...
使用::
    ...
    result = await run_routine('edit', kwargs={'path': 'foo.py', 'old_str': 'a', 'new_str': 'b'})

bridge 地址通过环境变量 ``ZERO_HTTP_ADDR`` 覆盖,
...
def _bridge_base() -> str:
    env = os.environ.get("ZERO_HTTP_ADDR")
    if env:
...
async def run_routine(name: str, kwargs: dict[str, Any] | None = None) -> Any:
    ...
    """
    agent_id = os.environ.get("ZERO_AGENT_ID", "user")
