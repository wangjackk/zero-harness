use routine::{
    protocol::{events::LIFECYCLE_START, JsonObject},
    server::grpc::{frame_to_json, json_to_frame, routine::routine_service_client::RoutineServiceClient},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tx.send(json_to_frame(&start)).await?;

    while let Some(message) = inbound.message().await? {
        let event = frame_to_json(&message);
